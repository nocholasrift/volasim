...
void Cylinder::buildVerticesSmooth() {
  ...
  for (int i = 0; i <= stackCount; ++i) {
    ...
    radius =
        baseRadius + (float)i / stackCount * (topRadius - baseRadius);  // lerp
    ...
    for (int j = 0, k = 0; j <= sectorCount; ++j, k += 3) {
      ...
      addNormal(sideNormals[k], sideNormals[k + 1],
...
void Cylinder::buildVerticesFlat() {
  ...
  for (i = 0; i < stackCount; ++i) {
    ...
    for (j = 0; j < sectorCount; ++j, ++vi1, ++vi2) {
      ...
      n = computeFaceNormal(v1.x, v1.y, v1.z, v3.x, v3.y, v3.z, v2.x, v2.y,
                            v2.z);
...
void Cylinder::buildInterleavedVertices() {
  ...
  for (i = 0, j = 0; i < count; i += 3, j += 2) {
    ...
    interleavedVertices.insert(interleavedVertices.end(), &vertices[i],
                               &vertices[i] + 3);

    //interleavedVertices.push_back(normals[i]);
    //interleavedVertices.push_back(normals[i+1]);
    //interleavedVertices.push_back(normals[i+2]);
    interleavedVertices.insert(interleavedVertices.end(), &normals[i],
                               &normals[i] + 3);

    //interleavedVertices.push_back(texCoords[j]);
    //interleavedVertices.push_back(texCoords[j+1]);
    interleavedVertices.insert(interleavedVertices.end(), &texCoords[j],
                               &texCoords[j] + 2);
  }
}
...
///////////////////////////////////////////////////////////////////////////////
std::vector<float> Cylinder::computeFaceNormal(float x1, float y1,
                                               float z1,  // v1
                                               float x2, float y2,
                                               float z2,  // v2
                                               float x3, float y3,
