...
void EventDispatcher::addEventListener(EventListener* l, std::string eventType){
  ...
  std::cout << "finding..." << eventType << std::endl;
  if(listeners.find(eventType) == listeners.end()){
    std::cout << "event type does not exist, adding " << eventType << std::endl;
    auto tuple = std::pair<std::string, std::vector<EventListener*>>(eventType, std::vector<EventListener*>());
