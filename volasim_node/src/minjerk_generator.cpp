...
vola::trajectory_t MinJerkGenerator::get_trajectory(
    const Eigen::Vector3d& curr_pos, const Eigen::Vector3d& target_pos,
    double duration, double step_size) {
  vola::trajectory_t traj;

  if (duration <= 0 || step_size <= 0) {
    return traj;
  }
  ...
  for (size_t step = 0; step < sz; ++step) {
    ...
    for (int axis = 0; axis < kNumAxes; ++axis) {
      ...
      state.pos[axis] = a5 * t5 + a4 * t4 + a3 * t3 + a2 * t2 + a1 * t + a0;

      state.vel[axis] =
          5 * a5 * t4 + 4 * a4 * t3 + 3 * a3 * t2 + 2 * a2 * t + a1;

      state.acc[axis] = 20 * a5 * t3 + 12 * a4 * t2 + 6 * a3 * t + 2 * a2;
...
Eigen::Matrix<double, MinJerkGenerator::kDegree, MinJerkGenerator::kNumAxes>
MinJerkGenerator::get_traj_coefficients(const Eigen::Vector3d& curr_pos,
