...
Drone::Drone(const Eigen::Matrix3d& J_mat, double torque_const, double boom_length, double mass, double dt) : DynamicObject<N_state, M_input>(dt) {
  x_ = Eigen::Vector<double, N_state>::Zero();
  x_(6) = 1.0;
  u_ = Eigen::Vector<double, M_input>::Zero();
...
solver_ = std::make_unique<amrl::RungeKutta<N_state, M_input>>([this](const X_t& x, const U_t& u){
...
void Drone::update(const U_t& u, double dt) {
  x_ = solver_->step(x_, u, dt);
  // std::cout << "x_new: " << x_ << std::endl;
...
typename Drone::X_t Drone::dynamics(const X_t& x, const U_t& u){

  glm::vec3 p(x(0), x(1), x(2));
  glm::vec3 v(x(7), x(8), x(9));
  glm::vec3 w(x(10), x(11), x(12));

  glm::quat q(x(6), x(3), x(4), x(5));
...
// }
  ...
  //           << std::endl;
  // std::cout << "calculating v_dot " << v_dot[0] << " " << v_dot[1] << " " << v_dot[2] << std::endl;
